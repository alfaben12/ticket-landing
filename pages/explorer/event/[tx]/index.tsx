...
const ExplorerTicketDetailPage = () => {
    ...
    const { getEventByTx, event, isLoading, resetState } = useExplorer();

    useEffect(() => {
        if (tx) {
            getEventByTx(tx as string);
        }
    }, [tx]);
                        ...
                        >
                            <div className="p-4 md:p-12 text-center lg:text-left">
                                ...
                                <p className="pt-2 text-gray-600 text-xs lg:text-sm flex items-center justify-center lg:justify-start">
                                    ...
                                    {moment(event.data.event.startTime).format(
                                        "LLL"
                                    )}{" "}
                                    -{" "}
                                    {moment(event.data.event.endTime).format(
                                        "LLL"
                                    )}
                                </p>
